    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-20 items-center">
            ...
            <div class="hidden lg:flex items-center gap-6">
                ...
                <!-- Artikel -->
                {{-- <div class="relative" x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false">
                    <button class="flex items-center gap-1 text-base font-medium transition-colors py-2 cursor-pointer {{ Route::is('artikel*') ? 'text-church-gold' : 'text-church-dark/60 hover:text-church-dark' }}">
                        Artikel
                        <svg class="w-3 h-3 transition-transform" :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                    </button>
                    <div x-show="open" x-transition.opacity class="absolute top-full left-0 mt-1 w-56 bg-white rounded-2xl shadow-xl border border-black/5 p-2" x-cloak>
                        <a href="{{ route('artikel_teologis.index') }}" class="block w-full text-left px-4 py-2.5 rounded-xl text-base transition-colors hover:bg-church-warm/50 text-church-dark/70">Artikel Teologis</a>
                        <a href="#" class="block w-full text-left px-4 py-2.5 rounded-xl text-base transition-colors hover:bg-church-warm/50 text-church-dark/70">Artikel Humanis</a>
                        <a href="#" class="block w-full text-left px-4 py-2.5 rounded-xl text-base transition-colors hover:bg-church-warm/50 text-church-dark/70">Khotbah Minggu</a>
                        <a href="#" class="block w-full text-left px-4 py-2.5 rounded-xl text-base transition-colors hover:bg-church-warm/50 text-church-dark/70">Liturgi</a>
                    </div>
                </div> --}}
    ...
    <div x-show="mobileMenuOpen" class="lg:hidden bg-white border-b border-black/5" x-transition x-cloak>
        <div class="px-4 py-6 flex flex-col gap-2">
            ...
            {{-- <div>
                ...
                <div x-show="activeDropdown === 'warta'" class="ml-6 mt-1 flex flex-col gap-1 border-l-2 border-church-gold/20 pl-4" x-cloak>
            ...
            <!-- Artikel -->
            {{-- <div>
                <button @click="activeDropdown = activeDropdown === 'artikel' ? null : 'artikel'" class="w-full flex items-center justify-between p-3 rounded-xl text-base font-medium {{ Route::is('artikel*') ? 'bg-church-gold/10 text-church-gold' : 'text-church-dark/60' }}">
                    Artikel
                    <svg class="w-4 h-4 transition-transform" :class="activeDropdown === 'artikel' ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                </button>
                <div x-show="activeDropdown === 'artikel'" class="ml-6 mt-1 flex flex-col gap-1 border-l-2 border-church-gold/20 pl-4" x-cloak>
                    <a href="{{ route('artikel_teologis.index') }}" class="block w-full text-left py-2 text-sm text-church-dark/50">Artikel Teologis</a>
                    <a href="#" class="block w-full text-left py-2 text-sm text-church-dark/50">Artikel Humanis</a>
                    <a href="#" class="block w-full text-left py-2 text-sm text-church-dark/50">Khotbah Minggu</a>
                    <a href="#" class="block w-full text-left py-2 text-sm text-church-dark/50">Liturgi</a>
                </div>
            </div> --}}
